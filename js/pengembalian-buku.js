...
let denda_keterlambatan = 0;
let denda_kondisi_buku = 0;
let total_denda = 0
let data_stok_buku = {};
...
db.ref("Peminjaman/").on("child_added", function (data) {
    ...
    document.getElementById("lihat-id-peminjaman").innerHTML += `
    <option value="${Value.id}">${Value.id}</option>
    `;

    document.getElementById("lihat-id-peminjaman").onchange = function () {
        let id_peminjaman = this.value;
        document.getElementById('lihat-table-daftar-buku').innerHTML = '';
        document.getElementById('denda-keterlambatan').value = '';
        ...
        db.ref('Peminjaman/' + id_peminjaman).once('value').then(function (snapshot) {
            ...
            db.ref('Anggota/' + id_anggota).once('value').then(function (snapshot) {
                ...
                document.getElementById('lihat-nama-anggota').value = nama;
            })
...
// ! Hitung Denda
document.getElementById('hitung-denda').addEventListener('click', (e) => {
    e.preventDefault();
    denda_keterlambatan = document.getElementById('denda-keterlambatan').value;
    denda_kondisi_buku = document.getElementById('denda-kondisi-buku').value;

    total_denda = parseInt(denda_keterlambatan) + parseInt(denda_kondisi_buku);

    document.getElementById('total-denda').value = total_denda;
})


// ! Menampilkan Buku ke dalam Combobox untuk cek Denda
db.ref("Buku/").on("child_added", function (data) {
    let Value = data.val();

    document.getElementById("cek-id-buku").innerHTML += `
    <option value="${Value.id}">${Value.id}</option>
    `;

    document.getElementById("cek-id-buku").onchange = function () {
        let id_buku = this.value;

        db.ref('Buku/' + id_buku).once('value').then(function (snapshot) {
            var title = (snapshot.val() && snapshot.val().title) || 'Tidak Ditemukan'
            var denda = (snapshot.val() && snapshot.val().denda) || 'Tidak Ditemukan'

            document.getElementById("cek-title-buku").value = title;
            document.getElementById("cek-denda-buku").value = denda;
        })
    };
})

// ! Create Pengembalian Buku
document.getElementById('btn-kembali').addEventListener('click', (e) => {
    document.getElementById('btn-kembali').disabled = true;
    document.getElementById('btn-kembali').innerHTML = 'Menambah pengembalian buku...';
    let id_peminjaman = document.getElementById('lihat-id-peminjaman').value;
    denda_keterlambatan = document.getElementById('denda-keterlambatan').value;
    denda_kondisi_buku = document.getElementById('denda-kondisi-buku').value;
    total_denda = document.getElementById('total-denda').value;

    let data_pengembalian = {
        id: counter,
        tgl_data_masuk: hari_ini,
        id_peminjaman: id_peminjaman,
        denda_keterlambatan: denda_keterlambatan,
        denda_kondisi_buku: denda_kondisi_buku,
        total_denda: total_denda,
        username: localStorage.getItem('username'),
    }

    db.ref("Pengembalian/" + counter).update(data_pengembalian);

    let data_status = {
        status: 'Sudah Dikembalikan',
    }

    db.ref("Peminjaman/" + id_peminjaman).update(data_status);
    ...
    document.getElementById('btn-kembali').innerHTML = 'Tambah Pengembalian Buku';
    window.alert('Tambah Pengembalian Buku Berhasil !');
    counter += 1;
...
// ! Create Pengembalian Buku dan Cetak Struk
document.getElementById('btn-kembali-struk').addEventListener('click', (e) => {
    document.getElementById('btn-kembali').disabled = true;
    document.getElementById('btn-kembali').innerHTML = 'Menambah pengembalian buku...';
    let id_peminjaman = document.getElementById('lihat-id-peminjaman').value;
    denda_keterlambatan = document.getElementById('denda-keterlambatan').value;
...
// ! Read Pengembalian
function readPengembalian(filter = 'tgl_data_masuk') {
    let db = firebase.database().ref("Pengembalian/").orderByChild(filter);
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table class="table table-bordered">
                            <tr>
                                <td style="width: 30%;">ID Peminjaman</td>
                                <td>${Value.id_peminjaman}</td>
                            </tr>
                            <tr>
                                <td style="width: 30%;">Denda</td>
                                <td>Rp. ${Value.total_denda}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <td style="width: 30%;">Tanggal Data Masuk</td>
                                <td>${Value.tgl_data_masuk}</td>
                            </tr>
                        </table>
                        <button type="button" class="btn btn-danger" onclick="printStruk('${Value.id}')">Cetak Struk</button>
                        <button type="button" class="btn btn-info" data-toggle="modal"
                        data-target="#modalDetailPengembalian" onclick="showPengembalian('${Value.id}')">Lihat Detail</button>
                    </div>
                </div>
            </div>
        </div>
...
// ! Detail Pengembalian
function showPengembalian(id_pengembalian) {
    document.getElementById('table-daftar-buku').innerHTML = ''
    total = 0

    db.ref('Pengembalian/' + id_pengembalian).once('value').then(function (snapshot) {
        let id = (snapshot.val() && snapshot.val().id) || 'Tidak Ditemukan'
        let id_peminjaman = (snapshot.val() && snapshot.val().id_peminjaman) || 'Tidak Ditemukan'
        let username = (snapshot.val() && snapshot.val().username) || 'Tidak Ditemukan'
        let denda_keterlambatan = (snapshot.val() && snapshot.val().denda_keterlambatan) || 'Tidak Ditemukan'
        let denda_kondisi_buku = (snapshot.val() && snapshot.val().denda_kondisi_buku) || 'Tidak Ditemukan'
        let total_denda = (snapshot.val() && snapshot.val().total_denda) || 'Tidak Ditemukan'
        let tgl_data_masuk = (snapshot.val() && snapshot.val().tgl_data_masuk) || 'Tidak Ditemukan'

        document.getElementById('tampil-id-pengembalian').value = id;
        document.getElementById('tampil-id-peminjaman').value = id_peminjaman;
        document.getElementById('tampil-denda-keterlambatan').value = denda_keterlambatan;
        document.getElementById('tampil-denda-kondisi-buku').value = denda_kondisi_buku;
        document.getElementById('tampil-total-denda').value = total_denda;
        document.getElementById('tampil-username').value = username;
        document.getElementById('tampil-tgl-data-masuk').value = tgl_data_masuk;
        ...
        db.ref('Peminjaman/' + id_peminjaman).once('value').then(function (snapshot) {
            ...
            let username_peminjaman = (snapshot.val() && snapshot.val().username) || 'Tidak Ditemukan'
            let pinjam = (snapshot.val() && snapshot.val().pinjam) || 'Tidak Ditemukan'
            let jumlah_pinjam = Object.keys(pinjam).length;

            document.getElementById('tampil-id-peminjaman-detail').value = id;
            ...
            document.getElementById('tampil-jumlah-pinjam').value = jumlah_pinjam + ' Eksemplar';
            document.getElementById('tampil-username-peminjaman').value = username_peminjaman;
        ...
        db.ref("Peminjaman/" + id_peminjaman + '/pinjam').on("child_added", function (data) {
            ...
            db.ref('Buku/' + id_buku).once('value').then(function (snapshot) {
                let id_buku = (snapshot.val() && snapshot.val().id) || 'Tidak Ditemukan'
                let title = (snapshot.val() && snapshot.val().title) || 'Tidak Ditemukan'
                let image = (snapshot.val() && snapshot.val().image) || 'Tidak Ditemukan'
...
// ! Report Struk
function printStruk(id_pengembalian) {
    db.ref('Pengembalian/' + id_pengembalian).once('value').then(function (snapshot) {
        let id = (snapshot.val() && snapshot.val().id) || 'Tidak Ditemukan'
        let username = (snapshot.val() && snapshot.val().username) || 'Tidak Ditemukan'
        let id_peminjaman = (snapshot.val() && snapshot.val().id_peminjaman) || 'Tidak Ditemukan'
        ...
        document.getElementById('struk-tgl-data-masuk').innerHTML = tgl_data_masuk;
        document.getElementById('struk-id-pengembalian').innerHTML = id;
        document.getElementById('struk-username').innerHTML = username;
        document.getElementById('struk-id-peminjaman').innerHTML = id_peminjaman;
        document.getElementById('struk-denda-keterlambatan').innerHTML = 'Rp ' + denda_keterlambatan;
        document.getElementById('struk-denda-kondisi-buku').innerHTML = 'Rp ' + denda_kondisi_buku;
        document.getElementById('struk-total-denda').innerHTML = 'Rp ' + total_denda;


        var doc = new jsPDF({ putOnlyUsedFonts: true, orientation: "portrait" });

        doc.setFontSize(14);
        doc.text("Struk Pengembalian Buku", 103, 20, null, null, "center");
        ...
        doc.setFontSize(10);
        doc.text("Jakarta, " + dateNow(), 170, 120, null, null, "center");
        doc.text(localStorage.getItem('username'), 170, 160, null, null, "center");
        doc.text("(Petugas Perpustakaan)", 170, 165, null, null, "center");
...
// ! Report Pengembalian
function print() {
    let db = firebase.database().ref("Pengembalian/");
    ...
    db.once('value', function (snapshot) {
        if (snapshot.exists()) {
            ...
            snapshot.forEach(function (data) {
                let id = data.val().id;
                let id_peminjaman = data.val().id_peminjaman;
                let denda_keterlambatan = data.val().denda_keterlambatan;
                if (denda_keterlambatan == 0) {
                    denda_keterlambatan = '-'
                }

                let denda_kondisi_buku = data.val().denda_kondisi_buku;
                if (denda_kondisi_buku == 0) {
                    denda_kondisi_buku = '-'
                }


                let total_denda = data.val().total_denda;
                if (total_denda == 0) {
                    total_denda = '-'
                }
                content += '<tr>';
                content += '<td>' + id + '</td>'; //column1
                content += '<td>' + id_peminjaman + '</td>';
                content += '<td>' + denda_keterlambatan + '</td>';//column2
                content += '<td>' + denda_kondisi_buku + '</td>';//column2
                content += '<td>' + total_denda + '</td>';//column2
                content += '</tr>';
            });
